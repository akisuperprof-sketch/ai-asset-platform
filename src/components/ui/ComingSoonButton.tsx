...
export function ComingSoonButton({ feature, className, children }: ComingSoonButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`${className} cursor-not-allowed opacity-60 hover:opacity-80 transition-all duration-300 relative flex items-center justify-center gap-2 group/soon`}
    >
      {children}
